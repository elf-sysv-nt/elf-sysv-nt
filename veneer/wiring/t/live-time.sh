#!/usr/bin/env bash
# WP-56's ninth live crossing: the bind loop (wire.c) resolves the time
...
# live-sockets.sh, live-locale.sh and live-posix.sh.
#
# The specimen (live-time.c) is freestanding and cross-built exactly as
# elfcall.c: it walks the auxv to AT_BASE, hands a PE-export resolver of
# wire.h's shape to __esn_wire_bind over the real wire-time.gen.c table,
# then calls one of the generated thunks (wire-time.gen.s: w00015 difftime)
# directly.
#
# time is 40 rows, all forwards, no shim, so its bind check is math's,
# stdlib's, sockets's, locale's and posix's shape: every row must resolve,
# missing == 0, not string's exactly-one-null. What it adds is a result
# class the earlier crossings never reached: difftime returns a double,
# handed back in %xmm0, from two integer time_t arguments, where ffs, abs,
# htonl and toascii returned integers in %rax and swab wrote through a
# caller pointer. difftime is also the slice's one argument-only NOSIGFE
# row -- the arithmetic difference of two calendar times -- where the
# clock, gmtime/localtime, mktime, strftime and timer calls read a clock,
# the tz rules or the thread pointer this harness never establishes. See
# live-time.c's header for the full account.
#
# Five checks, one bit each, so 31 is the only pass.
#
# Controls, the same shape as live-posix.sh's: a runtime that does not
...
# The specimen: the bind loop, the real time table, its generated thunk,
...
$cross-gcc -static -nostdlib -no-pie -ffreestanding -fcf-protection=none \
  ...
  -o "$tmp/live-time" \
  "$here/live-time-start.S" "$here/live-time.c" \
  "$wiring_dir/wire.c" "$wiring_dir/wire-time.gen.c" \
  "$wiring_dir/wire-time.gen.s" \
  || { bad "the specimen does not build"; }

if [ "$fail" != 0 ]; then say "verdict: no"; exit 1; fi

export ELFSYSV_STUB=$tmp/elfsysv-stub.exe
specimen=$(cygpath -w "$tmp/live-time")
...
if [ "$got" = 31 ]; then
  say "ok: the bind loop resolved the real time table (every forward filled) and difftime -- the slice's one argument-only row, and the first crossed forward whose answer returns in %xmm0 -- reached the real DLL (status 31)"
else
  bad "the live-time specimen returned $got, wanted 31:"
  sed 's/^/     /' "$tmp/live.out"
fi
