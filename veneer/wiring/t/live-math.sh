#!/usr/bin/env bash
# WP-56's first live crossing: the bind loop (wire.c) resolves the math
...
# The specimen here (live-math.c) is freestanding and cross-built exactly
# as elfcall.c: it walks the auxv to AT_BASE, but instead of resolving and
# calling exports itself, it hands a PE-export resolver of wire.h's shape to
# __esn_wire_bind over the real wire-math.gen.c table (34 rows, all
# NOSIGFE, so no full Cygwin process bring-up is needed), then calls three
# of the generated thunks (wire-math.gen.s) directly. Four checks, one bit
# each, so 15 is the only pass.
#
# Controls, the same shape as elfcall.sh's: a runtime that does not exist
...
# The specimen: the bind loop, the real math table, its generated thunks,
...
$cross-gcc -static -nostdlib -no-pie -ffreestanding -fcf-protection=none \
  ...
  -o "$tmp/live-math" \
  "$here/live-math-start.S" "$here/live-math.c" \
  "$wiring_dir/wire.c" "$wiring_dir/wire-math.gen.c" \
  "$wiring_dir/wire-math.gen.s" \
  || { bad "the specimen does not build"; }

if [ "$fail" != 0 ]; then say "verdict: no"; exit 1; fi

export ELFSYSV_STUB=$tmp/elfsysv-stub.exe
specimen=$(cygpath -w "$tmp/live-math")
...
if [ "$got" = 15 ]; then
  say "ok: the bind loop resolved the real math table and the wired thunks reached the real DLL (status 15)"
else
  bad "the live-math specimen returned $got, wanted 15:"
  sed 's/^/     /' "$tmp/live.out"
fi
