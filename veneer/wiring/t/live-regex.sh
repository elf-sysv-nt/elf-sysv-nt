#!/usr/bin/env bash
# WP-56's twentieth live crossing, crossed by its bind alone: the bind loop
# (wire.c) resolves the regex slice's real table against a real elfsysv1.dll,
# and no regex body is called.
#
...
# through live-threads.sh.
#
# The specimen (live-regex.c) is freestanding and cross-built exactly as
# elfcall.c: it walks the auxv to AT_BASE, hands a PE-export resolver of
# wire.h's shape to __esn_wire_bind over the real wire-regex.gen.c table,
# then reads the table the bind filled and the DLL's own PE header -- never a
# libc datum, and never a generated thunk.
#
# regex is 5 rows, all forwards and no shim: the POSIX regular-expression
# surface regcomp, regerror, regexec and regfree. It crosses with an empty
# unresolved set, the shape memory's, io-mux's and threads' crossings had. The
# one thing new is a compat pair: four rows version at GLIBC_2.2.5, but regexec
# carries two rows, one at GLIBC_2.2.5 and one at GLIBC_2.3.4 -- the pair glibc
# shipped when the regexec ABI changed. The tag lives in the thunk's .symver,
# not in the export name the bind resolves, so both regexec rows resolve to the
# single Cygwin regexec export and the bind fills both. So the slice needs no
# shim, and the finding is the absence of one across a slice with a compat pair
# -- no System V disposition as signal had, no struct translation as
# filesystem's stat family had.
#
# Crossed by its bind alone, not by call: no regex row is safely callable here.
# regcomp compiles a pattern into a malloc'd buffer, and regexec and regfree
# walk it; Cygwin's allocator is SIGFE and a freestanding harness never brought
# it up. See live-regex.c's header.
#
...
#   0x04 the resolver discriminates and the compat-pair finding holds: regexec
#        resolves, a junk name does not, and both regexec rows (2.2.5 and
#        2.3.4) bound to exactly that one export
#   0x08 regcomp, regexec and regfree reach three distinct bodies
...
# The specimen: the bind loop, the real regex table and its generated thunks
...
$cross-gcc -static -nostdlib -no-pie -ffreestanding -fcf-protection=none \
  ...
  -o "$tmp/live-regex" \
  "$here/live-regex-start.S" "$here/live-regex.c" \
  "$wiring_dir/wire.c" "$wiring_dir/wire-regex.gen.c" \
  "$wiring_dir/wire-regex.gen.s" \
  || { bad "the specimen does not build"; }

if [ "$fail" != 0 ]; then say "verdict: no"; exit 1; fi

export ELFSYSV_STUB=$tmp/elfsysv-stub.exe
specimen=$(cygpath -w "$tmp/live-regex")
...
if [ "$got" = 31 ]; then
  say "ok: the bind left no row unresolved (regex needs no shim, the whole slice binds across its compat pair), every filled slot lands inside the DLL's mapped image, the resolver discriminates and the compat-pair finding holds (regexec resolves, a junk name does not, and both regexec rows at GLIBC_2.2.5 and GLIBC_2.3.4 bound to exactly that one export), regcomp/regexec/regfree reach distinct bodies, and a second bind is idempotent -- the bind-only crossing holds and no regex body is entered, since the slice offers a freestanding harness none it may safely call (status 31)"
else
  bad "the live-regex specimen returned $got, wanted 31:"
  sed 's/^/     /' "$tmp/live.out"
fi
