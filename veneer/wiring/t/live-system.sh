#!/usr/bin/env bash
# WP-56's twenty-fourth and last live crossing, crossed by its bind alone: the
# bind loop (wire.c) resolves the system slice's real table against a real
# elfsysv1.dll, and no system body is called.
#
...
# through live-io.sh.
#
# The specimen (live-system.c) is freestanding and cross-built exactly as
# elfcall.c: it walks the auxv to AT_BASE, hands a PE-export resolver of
# wire.h's shape to __esn_wire_bind over the real wire-system.gen.c table, then
# reads the table the bind filled and the DLL's own PE header -- never a libc
# datum, and never a generated thunk.
#
# system is 6 rows, every one a forward and no shim: libc's machine-and-kernel
# enquiries. Five from sys/sysinfo.h -- get_avphys_pages, get_nprocs,
# get_nprocs_conf, get_phys_pages and sysinfo -- and one from sys/utsname.h,
# uname. Each is a forward-same row at GLIBC_2.2.5 with no rename and no compat
# pair. It crosses with an empty unresolved set, the shape memory's, io-mux's,
# threads', regex's, syslog's, sysv-ipc's and io's crossings had.
#
# The one thing new is that the wired table and the slice are the same width:
# the demand ranking placed six names in this slice and the table wires all
# six. No name moved to librt as io's aio family did, and none is a stub the
# classification left to WP-53's ret, so the slice carries no residue for
# real-map.sh's partition to own. The finding is the whole slice binding whole.
#
# Crossed by its bind alone, not by call: no system row is safely callable
# here. uname, sysinfo and the page and processor counts read the machine
# through Cygwin's subsystem, which is SIGFE and reaches the fhandler and
# kernel-query layers; a freestanding harness never brought it up. See
# live-system.c's header. The bodies are left to diff-slice.sh's
# uname-sysinfo differential.
#
...
#   0x04 the resolver discriminates and the plain-forward finding holds: uname
#        resolves, a junk name does not, and the single uname row bound to
#        exactly that one export
#   0x08 uname and sysinfo reach two distinct bodies
#   0x10 a second bind is idempotent (no row null, every slot a fresh resolve)
#
# Controls, the same shape as live-io.sh's: a runtime that does not exist must
# refuse before entry, and a run with no runtime at all must come back with
...
# The specimen: the bind loop, the real system table and its generated thunks
...
$cross-gcc -static -nostdlib -no-pie -ffreestanding -fcf-protection=none \
  ...
  -o "$tmp/live-system" \
  "$here/live-system-start.S" "$here/live-system.c" \
  "$wiring_dir/wire.c" "$wiring_dir/wire-system.gen.c" \
  "$wiring_dir/wire-system.gen.s" \
  || { bad "the specimen does not build"; }

if [ "$fail" != 0 ]; then say "verdict: no"; exit 1; fi

export ELFSYSV_STUB=$tmp/elfsysv-stub.exe
specimen=$(cygpath -w "$tmp/live-system")
...
if [ "$got" = 31 ]; then
  say "ok: the bind left no row unresolved (all six wired machine-and-kernel enquiries bind, the slice needs no shim), every filled slot lands inside the DLL's mapped image, the resolver discriminates and the plain-forward finding holds (uname resolves, a junk name does not, and the single uname row bound to exactly that one export), uname/sysinfo reach distinct bodies, and a second bind is idempotent -- the bind-only crossing holds and no system body is entered, since the slice offers a freestanding harness none it may safely call (status 31)"
else
  bad "the live-system specimen returned $got, wanted 31:"
  sed 's/^/     /' "$tmp/live.out"
fi
