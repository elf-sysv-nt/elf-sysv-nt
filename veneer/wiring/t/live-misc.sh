#!/usr/bin/env bash
# WP-56's tenth live crossing: the bind loop (wire.c) resolves the misc
...
# unchanged from live-math.sh through live-time.sh.
#
# The specimen (live-misc.c) is freestanding and cross-built exactly as
# elfcall.c: it walks the auxv to AT_BASE, hands a PE-export resolver of
# wire.h's shape to __esn_wire_bind over the real wire-misc.gen.c table,
# then calls two of the generated thunks (wire-misc.gen.s: w00016 insque,
# w00019 remque) directly.
#
# misc is 33 rows, all forwards, no shim, so its bind check is math's,
# stdlib's, sockets's, locale's, posix's and time's shape: every row must
# resolve, missing == 0, not string's exactly-one-null. What it adds is the
# shape of what the crossed rows touch: insque and remque take two caller
# pointers and edit a doubly-linked queue through them, where difftime
# returned a double, swab wrote a byte buffer and the abs and byte-order
# families returned scalars. The queue node is the specimen's own struct
# with two leading pointer words -- the q_forw, q_back layout the
# primitives' contract fixes for every libc -- so nothing here depends on a
# header the cross toolchain and the real DLL might lay out differently. The
# rest of misc reads process, locale, reent or the kernel and is left for
# the process bring-up the SIGFE-fenced slices wait on. See live-misc.c's
# header for the full account.
#
# Five checks, one bit each, so 31 is the only pass.
#
# Controls, the same shape as live-time.sh's: a runtime that does not exist
...
# The specimen: the bind loop, the real misc table, its generated thunks,
...
$cross-gcc -static -nostdlib -no-pie -ffreestanding -fcf-protection=none \
  ...
  -o "$tmp/live-misc" \
  "$here/live-misc-start.S" "$here/live-misc.c" \
  "$wiring_dir/wire.c" "$wiring_dir/wire-misc.gen.c" \
  "$wiring_dir/wire-misc.gen.s" \
  || { bad "the specimen does not build"; }

if [ "$fail" != 0 ]; then say "verdict: no"; exit 1; fi

export ELFSYSV_STUB=$tmp/elfsysv-stub.exe
specimen=$(cygpath -w "$tmp/live-misc")
...
if [ "$got" = 31 ]; then
  say "ok: the bind loop resolved the real misc table (every forward filled) and insque/remque -- the slice's two argument-only rows, editing a caller-owned queue through two pointers -- reached the real DLL (status 31)"
else
  bad "the live-misc specimen returned $got, wanted 31:"
  sed 's/^/     /' "$tmp/live.out"
fi
