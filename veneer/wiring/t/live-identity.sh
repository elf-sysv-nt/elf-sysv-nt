#!/usr/bin/env bash
# WP-56's seventeenth live crossing, crossed by its bind alone: the bind loop
# (wire.c) resolves the identity slice's real table against a real
# elfsysv1.dll, and no identity body is called.
#
...
# through live-process.sh.
#
# The specimen (live-identity.c) is freestanding and cross-built exactly as
# elfcall.c: it walks the auxv to AT_BASE, hands a PE-export resolver of
# wire.h's shape to __esn_wire_bind over the real wire-identity.gen.c table,
# then reads the table the bind filled and the DLL's own PE header -- never a
# libc datum, and never a generated thunk.
#
# identity is 17 rows, all forwards and no shim: the passwd database, the group
# database, and the two supplementary-group calls. It crosses with an empty
# unresolved set, the shape memory's and process's crossings had. Every
# identity export glibc names, Cygwin exports under the same name, and unlike
# process there is no LFS *64 variant to reconcile: every row's export_name is
# its own plain symbol. So the slice needs no shim, and the finding is the
# absence of one in its plainest form -- no System V disposition as signal had,
# no struct translation as filesystem's stat family had, no *64 alias to lean
# on as process's resource-limit rows had.
#
# Crossed by its bind alone, not by call: no identity row is stateless. The
# passwd and group readers walk /etc/passwd, /etc/group and Cygwin's account
# mapping and keep an enumeration cursor; initgroups and setgroups mutate the
# process's supplementary-group set; every one is SIGFE. A freestanding harness
# brings none of that up. See live-identity.c's header.
#
...
#        resolves, a junk name does not, and the "getpwnam" row bound to
#        exactly that export
#   0x08 getpwnam, getgrgid and initgroups reach three distinct bodies
#   0x10 a second bind is idempotent (no row null, every slot a fresh resolve)
#
# Controls, the same shape as live-process.sh's: a runtime that does not exist
...
# The specimen: the bind loop, the real identity table and its generated thunks
...
$cross-gcc -static -nostdlib -no-pie -ffreestanding -fcf-protection=none \
  ...
  -o "$tmp/live-identity" \
  "$here/live-identity-start.S" "$here/live-identity.c" \
  "$wiring_dir/wire.c" "$wiring_dir/wire-identity.gen.c" \
  "$wiring_dir/wire-identity.gen.s" \
  || { bad "the specimen does not build"; }

if [ "$fail" != 0 ]; then say "verdict: no"; exit 1; fi

export ELFSYSV_STUB=$tmp/elfsysv-stub.exe
specimen=$(cygpath -w "$tmp/live-identity")
...
if [ "$got" = 31 ]; then
  say "ok: the bind left no row unresolved (identity needs no shim, the whole slice binds), every filled slot lands inside the DLL's mapped image, the resolver discriminates and the no-alias finding holds (getpwnam resolves, a junk name does not, and the \"getpwnam\" row bound to exactly that export), getpwnam/getgrgid/initgroups reach distinct bodies, and a second bind is idempotent -- the bind-only crossing holds and no identity body is entered, since the slice offers a freestanding harness none it may safely call (status 31)"
else
  bad "the live-identity specimen returned $got, wanted 31:"
  sed 's/^/     /' "$tmp/live.out"
fi
