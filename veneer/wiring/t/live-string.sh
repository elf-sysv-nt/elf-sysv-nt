#!/usr/bin/env bash
# WP-56's fourth live crossing, and the first over a slice that mixes
# forwards with a shim: the bind loop (wire.c) resolves the string slice's
# real table against a real elfsysv1.dll, and three of the slice's generated
# thunks are called for real on NT.
#
...
# unchanged from live-math.sh and live-runtime.sh.
#
# The specimen (live-string.c) is freestanding and cross-built exactly as
# elfcall.c: it walks the auxv to AT_BASE, and instead of resolving and
# calling exports itself, it hands a PE-export resolver of wire.h's shape to
# __esn_wire_bind over the real wire-string.gen.c table, then calls three of
# the generated thunks (wire-string.gen.s: w00026 ffs, w00027 ffsl,
# w00028 ffsll) directly.
#
# string is the first crossed slice that is not wholly NOSIGFE, and it
# sharpens what NOSIGFE buys a freestanding specimen. memcpy and strverscmp
# are NOSIGFE too, but their bodies consult Cygwin's reent/TLS structure this
# harness never set up, so they return garbage here; the ffs family are pure
# register bit-scans that stand alone, so they cross cleanly. The reent-
# touching rows are left for the process bring-up the SIGFE slices wait on.
# See live-string.c's header for the full account.
#
# Unlike math, string's table also carries one shim (__errno_location). So
# the bind check is sharper than math's missing == 0: every forward must
# resolve and exactly the one shim must be left null, the forward/shim split
# the slice is built on made visible live. Four checks, one bit each, so 15
# is the only pass.
#
# Controls, the same shape as live-math.sh's: a runtime that does not exist
# must refuse before entry, and a run with no runtime at all must come back
# with none of the bits set, proving they are the runtime's and not
# accidents.
#
...
# The specimen: the bind loop, the real string table, its generated thunks,
...
$cross-gcc -static -nostdlib -no-pie -ffreestanding -fcf-protection=none \
  ...
  -o "$tmp/live-string" \
  "$here/live-string-start.S" "$here/live-string.c" \
  "$wiring_dir/wire.c" "$wiring_dir/wire-string.gen.c" \
  "$wiring_dir/wire-string.gen.s" \
  || { bad "the specimen does not build"; }

if [ "$fail" != 0 ]; then say "verdict: no"; exit 1; fi

export ELFSYSV_STUB=$tmp/elfsysv-stub.exe
specimen=$(cygpath -w "$tmp/live-string")
...
if [ "$got" = 15 ]; then
  say "ok: the bind loop resolved the real string table (every forward filled, the one shim __errno_location left null) and the ffs-family thunks reached the real DLL (status 15)"
else
  bad "the live-string specimen returned $got, wanted 15:"
  sed 's/^/     /' "$tmp/live.out"
fi
