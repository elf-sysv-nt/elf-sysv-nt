...
and the script reruns. This spike feeds `doc/design/target-definition.md`'s six
values.
...
the primary root against it, `measure-shape.sh` measured the same 41 ELF files;
the transcript is `results-2026-08-31.txt`. The only difference from 2026-08-29
is the reporting tool: the primary root's readelf 2.47 labels 14 of the 41
`DYN` objects "Position-Independent Executable file" where 2.29 called all 41
"Shared object file" — same bytes, finer labels, 27 + 14 = 41. Every value the
spike feeds `doc/design/target-definition.md` reads identically. Closed.
