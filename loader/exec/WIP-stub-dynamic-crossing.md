...
image's GOT and PLT resolve against the runtime's exports, runs the main
image's `DT_INIT`/`DT_INIT_ARRAY`, and only then enters at `e_entry` over the
SysV stack the process builder already laid down.

The certification drives a cross-built dynamic specimen — an interp-bearing
`ET_EXEC` that calls across an object boundary into an ELF runtime and exits
with what the call returns — through the real stub and front end, the same way
the WP-41 exec-elf check drives its specimen, and holds the exit status to the
value only a correctly relocated cross-object call produces.
