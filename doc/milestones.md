...
| 51 | `spike/syscall-fs-census/` | 0012 § 8 says the userland N cannot take as shipped is a list: packages whose text carries a raw `syscall` outside glibc, plus the Go runtime. How long is the list, over every x86_64 package in Rocky 8.10? | DR-0102's rebuild scope and phase 4's worklist. Run 2026-09-06 (`results-2026-09-06.txt`, `finding=raw-syscall-confined-to-runtimes,glibc-carries-its-own`): 3780 packages of the worklist's 4855, 17846 ELF files, none errored. 1419 packages carry an `0f 05` byte pair somewhere in text and only 71 carry an instruction the disassembler confirms — the byte scan over-reports by a factor of twenty, which is why the confirmation step exists. 69 of the 71 are outside glibc, 19 carry a Go runtime, and the list is what 0012 § 8 predicted it would be: container and Go tooling (podman 120, buildah, skopeo, runc, golang-bin 393), sanitizer and instrumentation runtimes (the libasan/libtsan family, valgrind 142, dyninst-testsuite 8732), the dotnet runtimes, and three in `kernel-core` that no userland links. Nothing in the base userland — bash and coreutils carry byte pairs that disassembly does not confirm. glibc's own 745 are the gate's business, not the list's. Bounded: 78% of the worklist, so the count is a floor and the shape is the finding. |
...
| 53 | `spike/gcc-specs-file-cost/` | DR-0061 makes the granule-separable link a toolchain default by installing a `specs` file where gcc reads it at startup. The file appends one option to `*link:`. What does the driver stop passing when it is there? | DR-0061 and DR-0062, which cite the mechanism, and WP-15's exit criterion, which it silently defeats. Run 2026-09-07 (`finding=any-specs-file-drops-eh-frame-hdr-and-shared-libgcc`): two things, neither named in the file. `--eh-frame-hdr` goes, so the linker writes no `.eh_frame_hdr` and no `PT_GNU_EH_FRAME`, and `_Unwind_Find_FDE` has no tables to find through `dl_iterate_phdr` at run time -- a C++ exception thrown across a shared-library boundary terminates, while every artifact a reader thinks to check looks right. `-lgcc_s` becomes `-lgcc`, moving the unwinder from the shared object to the static archive, which is why `libstdc++.so`'s `_Unwind_GetTextRelBase@GCC_3.0` and its neighbours went unresolved. An empty file at the same path costs exactly the same two, so the loss follows the file's presence and not its contents and cannot be fixed by writing the file more carefully. Bounded to gcc 13.3.0 on one target. The mechanism was read from `gcc/gcc.cc` afterwards and is DR-0108's: an installed specs file is read *instead of* the driver's own `init_spec ()`, which is where both losses originate, so the repair is to generate the file from `-dumpspecs` rather than write it by hand. |
| 54 | `spike/link-default-layer/` | DR-0061 wants the granule default to hold whether or not a build system knows to ask, and DR-0108 leaves the layer that carries it open at tier 8. Two candidates survive: A, `LINK_SPEC` in the gcc target header, and D, `ELF_MAXPAGESIZE` for this target's vector in bfd. Does either survive with no specs file, reach a link the gcc driver never ran, and still let one link override it? | The operator's ruling, which the ladder says is measured before it is escalated, and DR-0061's own bzip2 example, whose Makefile passes no max-page-size. Written 2026-09-07; not yet run. The apparatus measures a prefix per candidate and reports one that has not been built as not-built rather than failing, so the baseline transcript is worth taking before either rebuild is spent. |
| 55 | `spike/gate-call-arms/` | The glibc port's `ENTER_KERNEL` has three arms and the third, whose own comment calls it a static link, is the one `nscd` takes -- and it references `_dl_sysinfo`, which exists only in the static libc, so the build fails. Does any compile-time flag tell a dynamically linked program's object apart from a static link's? | The repair for the port's build failure, and DR-0108-era assumptions about what the compiler knows. Run 2026-09-16 (`finding=program-and-static-objects-are-indistinguishable-at-compile-time`): no. The failing object and one destined for the static libc carry identical flags, neither `SHARED` nor `PIC`, and across the whole build `SHARED` never appears without `PIC` (3,568 with both, 168 PIC only, none SHARED only, 3,061 compile lines with neither). A third arm conditioned on `PIC` therefore cannot be written, which eliminates the repair that reads as obvious and leaves two that must work for a dynamic program and a static one alike: export `_dl_sysinfo` from libc, or reach the thread pointer as upstream i386 does. Extended the same day with the second question, which settles the fork: a static program calls `__sbrk` at `csu/libc-tls.c:151` and sets its thread pointer at line 192, so the thread-pointer candidate cannot be made correct either and one candidate is left. DR-0111 takes it at tier 1. Bounded to el8's 2.28 as this project configures it, and the survivor is inferred from two eliminations rather than demonstrated by a build. |
