...
still carries one. Where user code reaches that gate is settled too, and by
elimination rather than by preference: through `_dl_sysinfo`, a symbol libc
exports, because a program's objects and the static libc's are
indistinguishable at compile time, and because a static program calls the
kernel to place its own TLS before it has a thread pointer to read.
DR-0005 was written when the instruction was the thing
