...
That bound is the settlement. `doc/design/target-definition.md` states it, and
the documents that previously called the field a deliberate lie now say what it
claims instead.
...
`doc/history/elf-technical-breakdown.md`, whose second bridge leaves no
`syscall` instruction to catch, and it has not been tested against a package
that insists on inline syscalls, because no package has been built yet.
...
`doc/design/target-definition.md`, under the limit of the `linux` claim, and in
