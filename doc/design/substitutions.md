...
Each open row names the section of `doc/design/Verification-Plan.md` whose bar
it is substituting against, so that reading the row says what claim the
substitution weakens and not only what was swapped for what.

| # | Substitute | For | Against | Where | Burns it down |
|---|------------|-----|---------|-------|---------------|
| S2 | The flags Red Hat's `redhat-hardened-cc1` and `redhat-hardened-ld` specs files inject, spelled out (`-fPIE`, `-pie`) | Running the build under those specs files themselves, through rpm and its macro set | `doc/design/Verification-Plan.md` § The bar per conformance class, the class A bar on image shape | `spike/vendor-hardened-build/`, the build line `acceptance/packages.tsv` carries for bzip2, and `%optflags`/`%build_ldflags` in `toolchain/rpm/macros.elfsysvnt` | An acceptance run against a package built by real rpm macros on a real el8 root, matching the `e_type` and segment placement this substitution claims. Until then the claim is narrow by construction: it covers image shape, which follows from `-fPIE` and `-pie` alone, and nothing about the hardening the other optflags provide. |
| S3 | No annobin annotations | `-specs=redhat-annobin-cc1`, which loads a gcc plugin that stamps build provenance into `.gnu.build.attributes` | `doc/design/Verification-Plan.md` § The bar per conformance class, the same class A bar narrowed to build provenance | `toolchain/rpm/macros.elfsysvnt`'s `%optflags`; the gap is reported every run as `cflags_absent_from_ours` in `spike/vendor-hardened-build/` | Either building the annobin plugin for the cross toolchain and matching the vendor, or a package in the el8 set whose build or check reads the annotations and is shown not to need them. Neither has been done; nothing in this project reads them, which is a statement about this project rather than about el8. |
| S4 | The WSL2 kernel (`6.18.33.2-microsoft-standard-WSL2`) under a Rocky 8.10 userland | el8's own 4.18 kernel, which is what builds the auxv on the target | `doc/design/Verification-Plan.md` § The bar per conformance class, narrowed to the auxv's `AT_HWCAP` and `AT_HWCAP2` | `loader/exec/t/hwcap-diff.sh`, run through the `LINUX_REF_DISTRO` seam; DR-0081 | The same differential against an auxv built by a real 4.18 el8 kernel. The substitution runs in the strict direction and cannot hide a divergence: `HWCAP2_FSGSBASE` was added in v5.9, so el8's kernel reports `AT_HWCAP2` as `0x0` where the WSL kernel reports `0x2`, and this platform's `0x0` matches el8 exactly while differing from the reference by the one bit DR-0081 explains. `AT_HWCAP` is a `cpuid` word and is kernel-independent on one CPU. |
